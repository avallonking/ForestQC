...
def random_forest_classifierAC(labelled_data, grey_variants):
    # input: a dataset that has balanced sample size of good and bad variants
    # output: predicted good or bad variants from grey variants
    x, y = labelled_data.loc[:, ['Mean_DP','Mean_GQ','SD_DP','SD_GQ','Outlier_DP','Outlier_GQ']].values, labelled_data.loc[:, 'Good'].values
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.20,random_state=1)
    pipeline = Pipeline([('scl', StandardScaler()), ('rf', RandomForestClassifier(random_state=1,n_jobs=8,n_estimators=10))])
    pipeline.fit(x_train, y_train)
    precison_recall = precision_recall_fscore_support(y_true=y_test, y_pred=pipeline.predict(x_test))
    ...
    x_pred = grey_variants.loc[:, ['Mean_DP','Mean_GQ','SD_DP','SD_GQ','Outlier_DP','Outlier_GQ']].values
    y_pred = pipeline.predict(x_pred)
...
def classification(good, bad, grey):
  if good.shape[0] > bad.shape[0]:
    prediction = random_forest_classifierB(pd.concat([good.sample(n=bad.shape[0]), bad]), grey)
  elif good.shape[0] == bad.shape[0]:
    prediction = random_forest_classifierB(pd.concat([good, bad]), grey)
  else:
    prediction = random_forest_classifierB(pd.concat([bad.sample(n=good.shape[0]), good]), grey)
